...
def installVersions(site)
  ...
  negativeAssert << ")"
  puts(negativeAssert)
  system("find #{docsDir} -mindepth 1 -not -path '*/[@.]*' -type f -print0 | xargs -O -P 4 -I {} perl -pi -e 's/xref:\\/midpoint\\/reference\\/(#{negativeAssert})/xrefv:\\/midpoint\\/reference\\/#{defaultBranch.gsub("docs/","")}\\//g' {} \\;")
  system("find #{docsDir} -mindepth 1 -not -path '*/[@.]*' -type f -print0 | xargs -O -P 4 -I {} perl -pi -e 's/midpoint\\/reference\\/(#{negativeAssert})/midpoint\\/reference\\/#{defaultBranch.gsub("docs/","")}\\//g' {} \\;")
