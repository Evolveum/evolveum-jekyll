...
module VersionReader
  ...
  def installVersions(site)
    ...
    negativeAssert << ")"
    puts(negativeAssert)
    system("find #{docsDir} -mindepth 1 -not -path '*/[@.]*' -type f -exec perl -pi -e 's/xref:\\/midpoint\\/reference\\/(#{negativeAssert})/xrefv:\\/midpoint\\/reference\\/#{@config['defaultBranch'].gsub("docs/","")}\\//g' {} +")
    system("find #{docsDir} -mindepth 1 -not -path '*/[@.]*' -type f -exec perl -pi -e 's/midpoint\\/reference\\/(#{negativeAssert})/midpoint\\/reference\\/#{@config['defaultBranch'].gsub("docs/","")}\\//g' {} +")

    @config['versions'].each_with_index do |version, index|
      versionWithoutDocs = version.gsub("docs/","")
      ...
      if version != @config['defaultBranch']
      ...
      system("sed -i 's/:page-nav-title: Configuration Reference/:page-nav-title: \"#{@config["displayVersions"][index]}\"/g' #{mpPreDir}#{versionWithoutDocs}/docs/index.adoc")
