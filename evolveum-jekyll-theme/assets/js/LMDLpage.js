(function() {
    ...
    const updateList = function(data) {
        ...
        for (let i = 0; i < data.hits.hits.length && i < data.hits.total.value; i++) { // TODO
            ...
            listitems.push(`<tr>
        <th scope="row"><a href="https://github.com/Evolveum/docs/commits/master/${data.hits.hits[i]._source.gitUrl}">${title}</a>&nbsp;<a href="https://github.com/Evolveum/docs/commits/master/${data.hits.hits[i]._source.gitUrl}"github history><i class="far fa-external-link"></i></a><i data-toggle="tooltip" title="${contentStatus}" class="${contentTriangleClass}"></th>
